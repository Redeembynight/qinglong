...
https://raw.githubusercontent.com/WindFgg/Scripts/main/Scripts/ql_aiqicha.js

 手动抓取CK:https://passport.baidu.com/v2/?login 一个BDUSS=xxxxx;就行了 请安装axios依赖
 定时每天早上8点执行一次 8点半执行一次 账号多的请加大延迟 可能有一些任务跑不完 如:高级搜索 浏览监控日报 属正常现象 (百度抽风)  批量查询任务暂时无法完成
...
const headers = {
  "User-Agent": "aiinquiry/2.3.10 (iPhone; iOS 14.5; Scale/3.00) Ios (21) f166836845f9838345b6 aiqicha/2.3.10",
    ...
    switch (o.title) {
      ...
      case "CX12009"://浏览互动
        ...
        if (HomeQuestionres.status == 0) {
          qdetail = HomeQuestionres.data.list[Math.floor((Math.random() * HomeQuestionres.data.list.length))]
          nid = qdetail.nid
...
async function aqc () {
 try{
  ...
  if (aqcookieArr) {
    console.log("爱企查cookie数量：" + aqcookieArr.length)
    ...
    msg += "请填写百度爱企查cookies(同百度贴吧、百度)的BDUSS"
  }
  console.log(msg);
